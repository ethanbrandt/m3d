...
void ECS::update_world_recursive(EntityID entityID, bool parentDirty)
{
	...
	if (isDirty)
	{
		record.transform.localMatrix = compose_transform(record.transform.localPosition, record.transform.localEulerDegrees, record.transform.localScale);
...
glm::mat4 ECS::compose_transform(const glm::vec3 &pos, const glm::vec3 &eulerDeg, const glm::vec3 &scale)
{
	glm::mat4 mat = glm::mat4(1.0f);

	mat = glm::translate(mat, pos);

	mat = glm::rotate(mat, glm::radians(eulerDeg.x), glm::vec3(1, 0, 0));
	mat = glm::rotate(mat, glm::radians(eulerDeg.y), glm::vec3(0, 1, 0));
	mat = glm::rotate(mat, glm::radians(eulerDeg.z), glm::vec3(0, 0, 1));
...
bool ECS::decompose_transform(const glm::mat4 &matrix, glm::vec3 &outPos, glm::vec3 &outEulerDeg, glm::vec3 &outScale)
{
	glm::quat rot;
	glm::vec3 skew;
	glm::vec4 perspective;

	if (!glm::decompose(matrix, outScale, rot, outPos, skew, perspective))
		return false;

	rot = glm::normalize(rot);
	outEulerDeg = glm::degrees(glm::eulerAngles(rot));
	return true;
}
...
void ECS::destroy_entity_record(EntityID entityID)
{
	...
	if (it != records.end())
	{
		...
		for (auto& [key, val] : it->second.components)
		{
			auto jt = componentToEntity.find(key);
			if (jt != componentToEntity.end())
				componentToEntity.erase(jt);
		}
...
void ECS::remove_component(EntityID entityID, ComponentID componentID)
{
	...
	{
		componentIT->second->on_destroy(entityID);
...
std::set<ComponentID> ECS::get_all_components_of_type(EntityID entityID, const std::type_info& type) const
{
	auto entityIT = records.find(entityID);
	if (entityIT == records.end())
		return { };
...
glm::vec3 ECS::get_entity_local_rot(EntityID entityID)
{
...
{
	glm::vec3 eulerDegrees = records.at(entityID).transform.localEulerDegrees;
	...
	return -glm::normalize(glm::vec3(rot[2].x, rot[2].y, rot[2].z));
}
...
glm::vec3 ECS::get_entity_local_right(EntityID entityID)
{
	...
	return glm::normalize(glm::vec3(rot[0].x, rot[0].y, rot[0].z));
}
...
glm::vec3 ECS::get_entity_local_up(EntityID entityID)
{
	...
	return glm::normalize(glm::vec3(rot[1].x, rot[1].y, rot[1].z));
}
...
void ECS::set_parent_entity(EntityID child, EntityID parent, bool keepWorldTransform)
{
	...
	if (keepWorldTransform)
	{
		...
		decompose_transform(local, childRecord.transform.localPosition, childRecord.transform.localEulerDegrees, childRecord.transform.localScale);
...
void ECS::clear_parent_entity(EntityID child, bool keepWorldTransform)
{
	...
	if (keepWorldTransform)
	{
		decompose_transform(oldWorld, childRecord.transform.localPosition, childRecord.transform.localEulerDegrees, childRecord.transform.localScale);
...
void ECS::update(float deltaTime)
{
	for (auto& [entityID, entityRecord] : records)
		for (auto& [componentID, component] : entityRecord.components)
			component->update(entityID, deltaTime);
}
